"""立体匹配后端统一入口（s2m2 | waft | las2 | ffs | OpenCV）。
...
def get_backend(name: str):
    ...
    if name == "opencv_sgbm":
        from . import opencv_sgbm_inference

        return opencv_sgbm_inference
...
def load(backend: str, model_type: str, weights_dir: str, device: str, **kwargs):
    ...
    if backend == "opencv_sgbm":
        from . import opencv_sgbm_inference

        return opencv_sgbm_inference.load_opencv_sgbm(
            model_type, weights_dir, device, **kwargs
        )
...
def run(backend: str, model, left, right, device: str, **kwargs):
    ...
    if backend == "opencv_sgbm":
        ...
        return opencv_sgbm_inference.run_stereo_matching(
