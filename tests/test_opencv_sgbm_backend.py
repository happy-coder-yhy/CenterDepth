...
class OpenCVSGBMBackendTests(unittest.TestCase):
    def test_backend_registry_includes_opencv_sgbm(self):
        self.assertIn("opencv_sgbm", stereo_backend.BACKENDS)

    def test_backend_registry_resolves_opencv_sgbm(self):
        self.assertEqual(
            stereo_backend.get_backend("opencv_sgbm").__name__,
            "stereo_center.opencv_sgbm_inference",
        )

    def test_load_uses_classical_sgbm_defaults(self):
        model = stereo_backend.load("opencv_sgbm", "ignored", "", "cpu")

        self.assertEqual(model.min_disparity, 0)
        self.assertEqual(model.num_disparities, 128)
        self.assertEqual(model.block_size, 5)
        self.assertEqual(model.p1, 600)
        self.assertEqual(model.p2, 2400)

    def test_load_rejects_invalid_sgbm_parameters(self):
        with self.assertRaisesRegex(ValueError, "multiple of 16"):
            stereo_backend.load(
                "opencv_sgbm", "ignored", "", "cpu", sgbm_num_disparities=20
            )
        with self.assertRaisesRegex(ValueError, "odd"):
            stereo_backend.load(
                "opencv_sgbm", "ignored", "", "cpu", sgbm_block_size=10
            )
        with self.assertRaisesRegex(ValueError, "greater than p1"):
            stereo_backend.load(
                "opencv_sgbm", "ignored", "", "cpu", sgbm_p1=64, sgbm_p2=64
            )
